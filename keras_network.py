...
def create_model(neurons, dropout_prob):
...
def grid_search_cv():
    model = KerasClassifier(build_fn=create_model, X_transform_fn=margin_shape_texture_split, nb_epoch=50, batch_size=1, verbose=1)
    ...
    param_grid = [
        {
            "neurons" : [192],
            "dropout_prob" : np.linspace(0.0, 0.3, num=4)
        }
    ]
...
def run_kaggle():
    model = create_model(neurons=192, dropout_prob=0.2)
    ...
    model.fit([X_margin_train, X_shape_train, X_texture_train],
              ...
              nb_epoch=250, batch_size=1, verbose=1)
